...
- [x] `PROP-2`: Frontmatter required keys are present.
- [x] `PROP-8`: Every TC has a `VERIFIES` trace tag.
- [x] `PROP-9`: All TC IDs use strict format and were issued by `get_next_id.py`.
- [x] `TEST-1`: Coverage Traceability Index is represented in `test-plan-v1.md` and TC trace metadata.
- [x] `TEST-2`: Must Have FRs have executable TC details with boundary, environment, data, owner, GWT, and expected results.
- [x] `TEST-3`: Rule / Branch Inventory maps every AC/BR/NFR to TC IDs.
- [x] `TEST-3b`: Per-AC technique matrix is complete.
- [x] `TEST-4`: Functional/SIT coverage checklist is complete.
- [x] `TEST-5`: Test data and teardown/reset are declared for state-changing TCs.
- [x] `TEST-6`: Automation intent is declared without writing runnable automation code.
- [x] `TEST-7`: External QA handoff is N/A and local evidence needs are documented in the test plan.
- [x] `TEST-8`: Generated TSV companions must be exported after this file changes.
